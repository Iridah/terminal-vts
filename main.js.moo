...
document.body.addEventListener('htmx:afterOnLoad', function(evt) {
    const input = document.getElementById('rut_search');
    ...
    if (input && !input.dataset.maskApplied) {
        input.addEventListener('input', function(e) {
            ...
            // 2. Limitar largo máximo (8 cuerpo + 1 DV)
            if (value.length > 9) value = value.slice(0, 9);

            // 3. Formatear visualmente: XXXXXXXX-X
            if (value.length > 1) {
        ...
        console.log("🛡️ Akama: Máscara de RUT vinculada al buscador.");
    }
});
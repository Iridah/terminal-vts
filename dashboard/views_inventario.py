...
def inventario_view(request):
    ...
    auditorias = AuditoriaVTS.objects.annotate(
        margen_db=ExpressionWrapper(
            (F('precio_venta') - F('precio_costo') * (1 + factor)) /
            Coalesce(F('precio_venta'), 1.0),
...
def inventario_pdf_view(request):
    ...
    BASE_DIR   = '/mnt/storage/proyectos/VTS'
    PYTHON     = f'{BASE_DIR}/venv/bin/python'
    BACKUP_DIR = f'{BASE_DIR}/backups'
    os.makedirs(BACKUP_DIR, exist_ok=True)

    timestamp = dt.now().strftime('%Y%m%d_%H%M%S')
    ruta_pdf  = f'{BACKUP_DIR}/inventario_{timestamp}.pdf'
    resultado = subprocess.run(
    [PYTHON, 'manage.py', 'generar_inventario_pdf', '--output', ruta_pdf],
    cwd=BASE_DIR,
    capture_output=True,
    text=True
    )
    ...
    if resultado.returncode != 0 or not os.path.exists(ruta_pdf):
        ...
        return HttpResponseServerError(
            f"Error generando PDF:<br><pre>{resultado.stderr}</pre>"
        )

    ...
    def handle(self, *args, **options):
        ...
        with open(csv_path, encoding='utf-8-sig') as f:
            ...
            for fila in reader:
                ...
                precio  = parse_monto(precio_str) if precio_str else 0.0
        ...
        self.stdout.write(
            f"\nCSV: {len(productos_por_itemid)} productos madre, "
            f"{len(variantes_data)} variantes"
        )

        # ── 3. ANÁLISIS (siempre corre, dry-run o no) ────────────
        stats = {
        ...
        for datos in variantes_data:
            ...
            if len(partes) >= 4:
                chevron = partes[3]
                if len(chevron) != 4 and chevron not in TALLAS_VALIDAS:
                    stats['inconsistencias'].append(
                        f"SKU variante inválido (chevron != 4 chars): {datos['sku']}"
                    )
                    continue

            padre = AuditoriaVTS.objects.filter(producto=datos['nombre']).first()
            ...
            if not padre:
                ...
                if not padre and sku_madre:
                    stats['prod_creados'] += 1
                    stats['inconsistencias'].append(
                        f"Padre inexistente, se creará: {sku_madre} ({datos['nombre']})"
                    )
                    if not dry_run:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    var.producto        = padre
                    if parse_monto(var.precio_venta) == 0 and datos['precio'] > 0:
        ...
        if stats['inconsistencias']:
            self.stdout.write(self.style.WARNING(
                f"\n  Inconsistencias ({len(stats['inconsistencias'])}):"
            ))
